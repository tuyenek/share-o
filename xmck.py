...
def process_jobs(xsmm_token, fb_data, delay_time):
    url_get_job = "https://xsmm.net/api/taskapi/tasks?type=facebook_like"
    headers = {"Authorization": f"Bearer {xsmm_token}", "Content-Type": "application/json"}
    
    print("\n" + " BẮT ĐẦU CHẠY JOB (TARGET_ID MODE) ".center(60, "="))
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    print(" [API] Lỗi định dạng JSON.")
                    time.sleep(5)
                    continue

                # --- FIX LỖI 'str' object has no attribute 'get' TẠI ĐÂY ---
                
                # Trường hợp 1: API trả về thông báo lỗi (Dictionary)
                ...
                # Trường hợp 2: API trả về List rỗng (Hết job)
                if isinstance(jobs, list) and not jobs:
                    print(" [Wait] Hết nhiệm vụ, nghỉ 10s...        ", end="\r")
                    time.sleep(10)
                    continue
                
                # Trường hợp 3: API trả về List nhiệm vụ (Đúng chuẩn)
                if isinstance(jobs, list):
                    print(f"\n [INFO] Tìm thấy {len(jobs)} job. Bắt đầu làm...")

                    for job in jobs:
                        if consecutive_errors >= MAX_ERRORS: break
                        
                        # Kiểm tra kỹ item trong list có phải dict không
...
def main():
    ...
    # 2. Login Facebook bằng Cookie
    uid_map = get_list_accounts_xsmm(token_xsmm)
    
    saved_cookie = load_json(COOKIE_FB_FILE)
    cookie_fb = ""
    print("\n" + " CẤU HÌNH TÀI KHOẢN FACEBOOK ".center(60, "="))
    
    if saved_cookie:
        print(f" [Gợi ý] Tài khoản cũ: {saved_cookie.get('name')} (UID: {saved_cookie.get('uid')})")
        if input(" [?] Dùng lại cookie cũ? (y/n): ").lower() == 'y':
            cookie_fb = saved_cookie.get('cookie')

    if not cookie_fb:
        cookie_fb = input(" [+] Nhập Cookie Facebook (c_user=...; xs=...): ").strip()

    print(" [Wait] Đang đăng nhập Facebook bằng Cookie...")
    fb_data = facebook_info(cookie_fb)

    if fb_data['success']:
        print(f" [OK] Đăng nhập thành công: {fb_data['name']} | UID: {fb_data['user_id']}")
        
        save_json(COOKIE_FB_FILE, {
            "cookie": cookie_fb, 
            "name": fb_data['name'], 
            "uid": fb_data['user_id']
        })
        ...
        if system_id:
            if set_active_account(token_xsmm, system_id):
                ...
                process_jobs(token_xsmm, fb_data, delay)
        else:
            print(" [ERR] Không tìm thấy System ID XSMM.")
    else:
        print(" [DIE] Cookie không hợp lệ hoặc bị Checkpoint.")
